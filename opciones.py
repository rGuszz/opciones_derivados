...
def arbol_alpha(arbol_subyacente, K, r, T, N, u, d, tipo="call"):
    ...
    for i in range(N-1, -1, -1): # El índice empieza en el numero de periodos menos 1 y termina en 0, disminuyendo de uno en uno
        for j in range(i+1): # Empieza en cero y se detiene hasta que  llega al último nodo que valua
            X_u = arbol[j, i+1] # Valor del derivado cuando sube
            X_d = arbol[j+1, i+1] # Valor del derivado cuando baja
            alpha = (X_u - X_d)/(arbol_subyacente[j, i+1] - arbol_subyacente[j+1, i+1])
            arbol[j, i] = alpha # Calculo de beta
    
    return arbol # Regresamos el árbol con todas las valuaciones

def arbol_beta(arbol_subyacente, K, r, T, N, u, d, tipo="call"):
    ...
    for i in range(N-1, -1, -1): # El índice empieza en el numero de periodos menos 1 y termina en 0, disminuyendo de uno en uno
        for j in range(i+1): # Empieza en cero y se detiene hasta que  llega al último nodo que valua
            X_u = arbol[j, i+1] # Valor del derivado cuando sube
            X_d = arbol[j+1, i+1] # Valor del derivado cuando baja
            beta = B*(X_u - (((X_u - X_d)*arbol_subyacente[j, i+1])/(arbol_subyacente[j, i+1] - arbol_subyacente[j+1, i+1])))
            arbol[j, i] = beta # Calculo de beta
...
with coluu2:
    c1, c2 = st.columns(2)
    with c2:
            opc1 = st.selectbox("Ver gráfica subyacente",options=["Gráfica Subyacente", "Gráfica Derivado"])
    with c1:
...
with colu2:
    ...
    if opcion == "Call Europea":
        ...
        if opc1 == "Gráfica Derivado":
         ...
         alpha = arbol_alpha(arbol_sub, K, r, T, N, u, d, tipo="call")
